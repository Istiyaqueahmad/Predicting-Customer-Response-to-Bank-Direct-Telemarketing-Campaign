...
// MAGIC                     .option("inferShema", "true")
// MAGIC                     .option("delimiter", ";")
// MAGIC                      .load("/FileStore/tables/bank-3.csv")     
...
// MAGIC %scala
// MAGIC
// MAGIC bankDF.createOrReplaceTempView("Bank3Data")
...
// MAGIC %sql
// MAGIC
// MAGIC select y, count(y) as age_1 from Bank2Data group by y;
...
// MAGIC
// MAGIC
// MAGIC select marital,count(marital)as ppp from Bank2Data group by marital order by ppp asc
...
// MAGIC %sql
// MAGIC
// MAGIC select  mean(age) from Bank3Data group by age;
...
// MAGIC %sql
// MAGIC
// MAGIC select education, count(education) from Bank2Data group by education;
...
// MAGIC %sql 
// MAGIC
// MAGIC select housing, count(housing) from BankData-3 group by housing;
...
// MAGIC %sql
// MAGIC
// MAGIC select housing, count(housing) from Bank2Data group by housing;
...
// MAGIC   Seq((0, "a"), (1, "b"), (2, "c"), (3, "a"), (4, "a"), (5, "c"))
// MAGIC ).toDF("id", "category")
// MAGIC
// MAGIC val indexer = new StringIndexer()
// MAGIC   .setInputCol("category")
// MAGIC   .setOutputCol("categoryIndex")
// MAGIC
// MAGIC val indexed = indexer.fit(df).transform(df)
// MAGIC
// MAGIC display(indexed)
...
// MAGIC %scala
// MAGIC
// MAGIC val assembler = new VectorAssembler().setInputCols(Array("age", "duration", "campaign", "pdays", "previous", "empvarrate", "conspriceidx", "consconfidx", "euribor3m", "nremployed", "job_indexed", "marital_indexed", "education_indexed", "default_indexed", "housing_indexed", "loan_indexed", "contact_indexed", "month_indexed", "day_of_week_indexed", "poutcome_indexed")).setOutputCol("features")
// MAGIC
...
// MAGIC
// MAGIC val evaluator = new BinaryClassificationEvaluator().setLabelCol("trueLabel").setRawPredictionCol("rawPrediction").setMetricName("areaUnderROC")
// MAGIC val auc = evaluator.evaluate(prediction)
// MAGIC println("AUC = " + (auc))
